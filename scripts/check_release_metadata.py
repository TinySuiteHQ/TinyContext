...
GITHUB_REPOSITORY = "TinySuiteHQ/TinyContext"
GITHUB_URL = f"https://github.com/{GITHUB_REPOSITORY}"
SERVER_NAME = "io.github.TinySuiteHQ/tinycontext"
PYPI_PACKAGE = "tinysuite-context"
OCI_REPOSITORY = "docker.io/marcellm01/tinycontext"
...
def check(expected_version: str | None = None) -> str:
    ...
    if server.get("version") != version:
        raise ValueError("server.json version does not match pyproject.toml")
    ...
    for label in ("Homepage", "Repository"):
        if project.get("urls", {}).get(label) != GITHUB_URL:
    ...
    if pypi.get("version") != version:
        raise ValueError("PyPI package version does not match pyproject.toml")
    ...
    if oci.get("identifier") != expected_oci:
        raise ValueError(
            f"OCI package identifier {oci.get('identifier')!r} must be "
            f"{expected_oci!r}"
        )

    marker = f"mcp-name: {SERVER_NAME}"
    if marker not in readme:
        raise ValueError(f"README.md must contain MCP ownership marker {marker!r}")
    source_label = f'org.opencontainers.image.source="{GITHUB_URL}"'
    if source_label not in dockerfile:
        raise ValueError(f"Dockerfile must contain source label {source_label!r}")
    server_label = f'io.modelcontextprotocol.server.name="{SERVER_NAME}"'
    if server_label not in dockerfile:
        raise ValueError(f"Dockerfile must contain MCP server label {server_label!r}")

    if not re.fullmatch(r">=3\.12", str(project.get("requires-python", ""))):
        raise ValueError("project.requires-python must be '>=3.12'")
    return version
